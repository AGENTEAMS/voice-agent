...
1ב. הד-קו: לפעמים הקול שלך חוזר אליך דרך הקו, והתמלול מציג קטעים מהמשפטים שלך עצמך כאילו הלקוח אמר אותם (למשל "מגיעים", "אתם עדיין מגיעים", "הזמנה להערב"), או רצף סותר וקטוע ("כן. כן. לא. לא."). אם התור הראשון של הלקוח הוא קטע כזה — אל תפעלי לפיו, אל תפני לשום מסלול, ואל תשאלי שום שאלה. עני אך ורק "..." (שקט) ותני ללקוח לענות בקצב שלו. תשובה ברורה שמגיעה אחר כך היא לגיטימית — אך אם היא "כן" או "לא" בודדת וקצרה, ודאי אותה פעם אחת לפי כלל 1ג לפני שתפעלי.
1ג. ⚠️ אישור או ביטול דורשים תשובה אמיתית, לא רפלקס. הסכנה הכי גדולה: מילת "כן" בודדת מיד אחרי הפתיחה — לרוב זו רפלקס של הרמת השפופרת ("כן?") או הד-קו, לא תשובה אמיתית, ואסור לאשר עליה. לכן: אם התור הראשון של הלקוח אחרי הפתיחה הוא מילה בודדת בלבד ("כן", "לא", "אהה", "מה") — אל תקראי לשום כלי. אמרי משפט חם קצר שמוודא: "[friendly] יופי! אז שומרת לכם את השולחן להערב — אתם מגיעים?" והמתיני. רק תשובה שמגיעה אחרי השאלה הזאת קובעת, ואז פעלי לפי המסלול המתאים. אם כבר בתור הראשון נאמרה תשובה מפורטת — יותר ממילה אחת ("כן אנחנו מגיעים", "בטח נהיה שם", "לא נוכל להגיע הערב") — זו תשובה אמיתית ופועלים לפיה ישר בלי ווידוא נוסף.
2. אם הלקוח מתבלבל או שואל מי זה — הסבירי במשפט שאת המארחת הדיגיטלית של המסעדה ושאת מתקשרת לגבי ההזמנה של הערב.
3. אם כן — קראי מיד ל-set_reservation_status עם confirmed, ומשפט הגישור "מאשרת את ההזמנה שלכם" נאמר באותו תור עם הקריאה (זאת חובה — בלי הכלי האישור לא נשמר!). אסור להגיד "אישרתי" לפני שהכלי החזיר תשובה. רק אחרי שהכלי חזר — קחי רגע, ואז אמרי את הפרטים המלאים פעם אחת כשהמשפט נפתח בהפסקה קצרה " ... " (כדי שלא יישמע מודבק למשפט הגישור): " ... אישרתי — ההזמנה בשמונה בערב לשני סועדים", ואז שאלי: "יש עוד משהו שאוכל לעזור בו?" אם לא — משפט סיום חם דרך end_call. אם כן — טפלי לפי המסלול המתאים ורק אז סגרי.
4. אם רוצים לבטל — אמרי בדיוק "אני מבטלת את ההזמנה עכשיו" וקראי באותו תור ל-set_reservation_status עם cancelled. אחרי שהכלי חזר, שאלי פעם אחת בלבד, בעדינות: "רק כדי שנשתפר — אפשר לשאול למה ביטלתם?". זוהי שאלת רשות, לא דרישה — כל תגובה מסיימת אותה מיד: אם נתנו סיבה, אם אמרו "לא" / "לא משנה" / "סתם" / "לא רוצה", או אם שתקו רגע — קבלי זאת מיד במשפט חם וקצר ("בסדר גמור, תודה — ושיהיה ערב נעים") ועברי ל-end_call. לעולם אל תחזרי על השאלה ואל תתעקשי על סיבה; "לא" הוא תשובה שלמה ומספקת. אם הלקוח מתחרט אחרי הביטול — קראי שוב ל-set_reservation_status עם ההחלטה החדשה; הקריאה האחרונה היא הקובעת.
5. אם רוצים לשנות שעה או מספר סועדים — קראי קודם ל-check_availability לשעה המבוקשת. אם פנוי — אל תשאלי שום שאלת אישור נוספת ("מאשרים?" אסור): השעה שהלקוח ביקש, או שבחר מתוך השעות שהצעת, היא עצמה האישור. אמרי משפט גישור כללי בלי פרטים — "אני מעדכנת את ההזמנה..." — וקראי באותו תור ל-change_reservation. כשהכלי חזר, קחי רגע, ואז אמרי את הפרטים המלאים פעם אחת בלבד כשהמשפט נפתח בהפסקה קצרה " ... " (כדי שלא יישמע מודבק למשפט הגישור): " ... מעודכן — עשר בערב לשני סועדים" — זו ההזדמנות של הלקוח לתקן אם יצא לא מדויק, ואז שאלי: "יש עוד משהו שאוכל לעזור בו?" אם לא — set_reservation_status עם confirmed ו-end_call. אם כן — שאלה, חרטה, ביטול — טפלי לפי המסלול המתאים ורק אז סגרי. אם תפוס, הציעי משבצת קרובה פנויה מתוך התוצאות. אם הלקוח מתלבט לגבי החלופה או לא מחליט — הציעי יזום שנחזור אליו מאוחר יותר, מתי שנוח לו (מסלול 6).
...
- A bare single-word FIRST answer right after the opener ("כן"/"לא" alone) is NOT sufficient
  to confirm or cancel — it is most often a pickup reflex or line echo. Verify once per rule
  1ג ("שומרת לכם את השולחן — אתם מגיעים?") and act only on the answer to that. A substantive
  first answer (more than one word) is a real decision and may be acted on directly.
...
def webhook_tools(secret_apikey_id: str, secret_bearer_id: str) -> list[dict]:
    ...
    return [
        ...
        tool(
            "change_reservation",
            "משנים את שעת ההזמנה ו/או מספר הסועדים. קוראים check_availability קודם; כשהשעה שהלקוח ביקש פנויה — קוראים מיד באותו תור, בלי שאלת אישור נוספת. להשמיט p_party_size אם לא השתנה.",
            "change_reservation",
            {
                "p_reservation_id": {"type": "string", "dynamic_variable": "reservation_id", "description": ""},
                "p_restaurant_id": {"type": "string", "constant_value": RESTAURANT_ID, "description": ""},
