...
agent/.venv/bin/python agent/make_reservation.py --name "תומר אלזם" --phone +972500000000 --time 20:30 --party 4
agent/.venv/bin/python agent/make_reservation.py --name "תומר אלזם" --phone +972500000000 --time 19:00 --party 2
cd stage && npm run dev                  # localhost:3000 (+ /tonight in a second tab)
```
- Transfer (`transfer_to_human`) rings **+972500000000** — set so stage-call tests
  (which ring +972500000000) make the *other* device ring. Callbacks never fire on
...
**Level 2 — Single live call (stage CTA):** phone +972500000000 rings.
...
```bash
python agent/call_and_verify.py --reservation <uuid> --to +972500000000
```
...
**CTA always works:** `stage/.env.local` sets `STAGE_CALL_TARGET=+972500000000`, so
...
**Level 3 — n8n batch + stage as observer:** phone +972500000000 rings (n8n allowlist).
1. Stage + `/tonight` open on the projector.
2. YOUR-N8N-INSTANCE.app.n8n.cloud → «Mika Voice Agent — Call Today's Pending Reservations» → **Execute
